...
(function ($) {
    ...
    $.fn.SetScroller = function (options) {
        ...
        options);
        var velocity = typeof options.velocity ==
        'number' ? parseInt(options.velocity) : 50;
        var direction = options.direction.toLowerCase();
        var startfrom = options.startfrom.toLowerCase();
        var loop = typeof options.loop ==
        'number' && options.loop > 0 ? parseInt(options.loop) : -1;
        var movetype = options.movetype.toLowerCase();
        //TODO: 'linear' or 'pingpong'
        ...
        function main() {
            ...
            if(direction=='horizontal'){
                if(startfrom!='right' && startfrom!='left'){
                    startfrom='right';
                };
                scrollHorizontal();
            }else{
                if(startfrom!='bottom' && startfrom!='top'){
                    startfrom='bottom';
                };
                scrollVertical();
            };
                            ...
                            switch(movetype){
                                case 'pingpong':
                                    if(startfrom == 'bottom'){
                                        ...
                                        if(parseInt(ob.position().top)+
                                            parseInt(ob.height()) >=
                                            parseInt(ob.parent().height())){
                                            startfrom='bottom';
                                            ob.stop();
                                            ob.trigger('marquee');
                                        };
