    ...
    public ArrayList<TrainingParticipantModel> getParticipantEmployeesList(int requestId) {
        ...
        try {
            ...
            while (_rs.next()) {
                ...
                currentTraining.setJobTitleName(_rs.getString("JOB_NAME"));
...
//            }
    ...
    public ArrayList<SelectItem> getPendingRequests() {
        ...
        try {
            ...
            while (_rs.next()) {
                SelectItem currentRequest = new SelectItem(
                        ...
                        _rs.getString("REQUEST_STATUS"),
                        _rs.getString("REQUESTER_ID") + " ON " +
     ...
     public ArrayList<SelectItem> getPendingRequestsList() {
        ArrayList<SelectItem> requestLists = new ArrayList<SelectItem>();
        try {
            ResultSet _rs = trainingNeedAssessementRequestEntity.selectPendingRequestsList(userName);
            while (_rs.next()) {
                SelectItem currentRequest = new SelectItem(
                        _rs.getString("NEED_ASSESSEMENT_REQUEST_ID") + "--" +
                        _rs.getString("REQUESTER_ID") + "--" +
    ...
    public ArrayList<TrainingParticipantModel> getApprovedParticipantsOfTraining(String budjetYear, int trainingType) {
        ...
        try {
            ...
            while (_rs.next()) {
                ...
                currentParticipant.setJobTitleName(_rs.getString("JOB_NAME"));
