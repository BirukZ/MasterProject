...
public class TerminationApprovePage extends AbstractPageBean {
    ...
    private void disableButtons() {
        ...
        chksuccesisor.setDisabled(true);
    ...
    private void enableButtons() {
        ...
        chksuccesisor.setDisabled(false);
    ...
    public boolean saveTerminationDecision() {
        ...
        if (txttermType.getValue().toString().equals("Death") && (drlDecision.getValue().toString().equals("6"))) {
            terminationApproveManager.changeEmployeeList(requesterId);
        } else if (txttermType.getValue().toString().equals("Other") || txttermType.getValue().toString().equals("discipline") || txttermType.getValue().toString().equals("Sick ness") || (txttermType.getValue().toString().equals("Injuri")) && (drlDecision.getValue().toString().equals("6"))) {
            terminationApproveManager.changeEmployeeListResign(requesterId);
        } else if (txttermType.getValue().toString().equals("Age Limit") && (drlDecision.getValue().toString().equals("6"))) {
            terminationApproveManager.changeEmployeeListRetirment(requesterId);
        }
        return terminationApproveManager.saveTerminationDecision(terminationRequestId, txtDeciderId.getValue().toString(), drlDecision.getValue().toString(), requestStatus, comment, terminator, terminationType, terminationCatagory, terminationDate, penalityPay, hasSuccesioser);
    }
    ...
    public void selectRequestListsByRequester_processValueChange(ValueChangeEvent vce) {
        ...
        txttermType.setValue(terminationType);
    }
//"TERMINATION_REQUEST_ID") + "--" +("REQUESTER_ID") + ("REQUEST_STATUS") ("DESCRIPTION"),
    ...
    public void selectMyHistoryLists_processValueChange(ValueChangeEvent vce) {
        if (vce != null) {
            ...
            decisionsMadeOnRequest = terminationApproveManager.getRequestHistory(terminationRequestId);
            disableButtons();
            selectRequestApprovableRequestLists.setValue(null);
    ...
    private boolean populateEmployeeTextComponents(String employeeId) {
        HashMap empInfoHashMaps = employeeManager.readEmployeeBasicInfo(employeeId);
        HashMap empInfoHashMap = terminationRequestManager.readEmployeeBasicInfo(employeeId);
        ...
        if (empInfoHashMap != null) {
            if (empInfoHashMap.get("employeeId").toString().equals("")) {//if no resultset found
                ...
                terminator = empInfoHashMaps.get("employeeId").toString();
                txtRequesterDepartment.setValue(empInfoHashMaps.get("employeeDepartmentPath"));
                txtRequesterFullName.setValue(empInfoHashMaps.get("employeeFullName"));
                txtRequesterId.setValue(empInfoHashMaps.get("employeeId"));
                txtRequesterPosition.setValue(empInfoHashMaps.get("employeePosition"));
                txtSalary.setValue(empInfoHashMap.get("EMPSALARY"));
    ...
    private boolean populateTerminationRequestComponents(int requestId) {
        ...
        if (terminationRequestModel != null) {
            if (terminationRequestModel.getTerminationRequestId() == -1) {
                ...
                terminationType = terminationRequestManager.getTypeOFTeminationTypeOne(terminationRequestModel.getTerminationType());
                txttermType.setValue(terminationType);
    ...
    private void clearTerminationRequestComponenets() {
        ...
        txttermType.setValue(null);
        txtaDescription.setValue(null);
        txtCalculatedPenal.setValue(null);
        txtDocReferenceNumber.setValue(null);
        drlTerminationCatagory.setValue(null);
    }
...
// </editor-fold >
    ...
    public void chksuccesisor_processValueChange(ValueChangeEvent vce)
    {
        if (chksuccesisor.isSelected())
        {
            btnCalculatePenality.setDisabled(true);
            penalityPay = 0;
            txtCalculatedPenal.setValue(penalityPay);
            hasSuccesioser = "Yes";
        } else {
            btnCalculatePenality.setDisabled(false);
            hasSuccesioser = "No";
...
//            } else
            ...
            if (txtAppliedDate.getValue().toString() != null) {
                date = (Date) formatter.parse(txtAppliedDate.getValue().toString());
                selectedAppliedDate = date;
            }
        } catch (Exception es) {
            es.printStackTrace();
        ...
        terminationDate = txtTerminationDate.getValue().toString();
 if (chksuccesisor.isSelected()) {
            btnCalculatePenality.setDisabled(true);
            penalityPay = 0;
            hasSuccesioser = "Yes";
        ...
        if ((terminationType.equalsIgnoreCase("On force retirement")) || (terminationType.equalsIgnoreCase("Age Limit")) || (terminationType.equalsIgnoreCase("discipline")) || (terminationType.equalsIgnoreCase("Death")))
        {
            penalityPay = 0;
            txtCalculatedPenal.setValue(penalityPay);
        } else if (dateDifference <= 0)
        {
            txtCalculatedPenal.setValue(txtSalary.getValue().toString());
            penalityPay = Double.valueOf(txtCalculatedPenal.getValue().toString());
...
//            txtCalculatedPenal.setValue(penality);
//            penalityPay = Double.valueOf(txtCalculatedPenal.getValue().toString());
        }
 }
...
//            } else 
        ...
        if ((selectedAppliedDate.after(selectedTerminationDate) && selectedTerminationDate.before(selectedAppliedDate))) {
