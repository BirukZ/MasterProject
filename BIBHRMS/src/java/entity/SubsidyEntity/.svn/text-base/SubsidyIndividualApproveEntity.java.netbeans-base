...
import manager.SubsidyManager.SubsidyIndividualApproveManager.SubSidyDecisionModel;
 ...
 * @author mekete
 */
public class SubsidyIndividualApproveEntity
    ...
    public SubsidyIndividualApproveEntity() {
    }

    public boolean insertTerminationDecision(SubSidyDecisionModel terminationDecisionModel) throws SQLException {
        String _insertQuery = "INSERT INTO HR_SUBSIDY_PROCESSED " +
                " (ID,REQUEST_ID, PROCESSED_BY,DECISION, " +
                "  COMMENT_GIVEN, RECORDED_BY, TIME_STAMP)" +
                " VALUES (Hr_subsidy_Approve.NEXTVAL,?,?,?,?,?,?) ";
        String _updateQuery = "UPDATE HR_SUBSIDY_REQUEST SET REQUEST_STATUS=? WHERE SUBSIDY_ID=?";
...
//            }
    ...
    public ResultSet selectApprovableRequest(String userName) throws SQLException {
        ...
        String _selectQuery = "SELECT * FROM HR_SUBSIDY_REQUEST WHERE REQUEST_STATUS  IN (" + _innerSelectQuery + ")";
    ...
    public ResultSet selectApprovedRequest(String userName) throws SQLException {
        String _selectQuery = "SELECT TR.*, (E.FIRST_NAME || ' ' || E.MIDDLE_NAME || ' ' || E.LAST_NAME) AS FNAME " +
                "FROM HR_SUBSIDY_REQUEST TR, HR_EMPLOYEE_INFO E " +
...
//            _ps.setString(1, userName);
    ...
    public ResultSet selectMyDecisionsOnRequest(String userName) throws SQLException {
        String _selectQuery = "SELECT * FROM HR_SUBSIDY_REQUEST WHERE SUBSIDY_ID IN " +
                "(SELECT REQUEST_ID FROM HR_SUBSIDY_PROCESSED WHERE RECORDED_BY =?)";
        try {
            _con = getConnection();
            _ps = _con.prepareStatement(_selectQuery);
            _ps.setString(1, userName);
