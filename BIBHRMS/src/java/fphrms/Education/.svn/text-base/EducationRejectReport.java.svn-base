...
import manager.educationManager.RejectedEducReportManager;
 ...
 */
public class EducationRejectReport extends AbstractPageBean {
    ...
    RejectedEducReportManager reportManager = new RejectedEducReportManager();
    ...
    private boolean populateCourseTableComponents() {
        // int tableSize;
        ArrayList<HashMap> empInfoHashMap = reportManager.getRejectedRequest();
        if (empInfoHashMap != null) {
            ...
            for (HashMap hmTc : empInfoHashMap) {
               ...
               // appliedDate = hmTc.get("applied_date").toString();
                ...
                reasonOfRejection = hmTc.get("comment_given").toString();
                educationRequsetList.add(new ReportModel(requesterId, requesterName, age , sex, hireDate, jobPostion, dpartement,team,location,currentEducation,currentEducationrank,result, requestedEducation, requestedEuctionrank,institutionName, institutionAccredited,reasonOfRejection));
                tableSize++;
                    }
            return true;
        } else {
