...
import manager.powerDeligationManager.PowerDelegationApproveReportManager;
import manager.powerDeligationManager.PowerDelegationModel;
import manager.powerDeligationManager.PowerDelegationModel.PowerDegationApproveModel;
 ...
 */
public class PowerDelegationApproveReport extends AbstractPageBean {
     ...
     */
    public PowerDelegationApproveReport() {
    }
    ...
    public void init() {
        ...
        try {
            ...
            log("PowerDelegationApprove Initialization Failure", e);
    ...
    protected ApplicationBean1 getApplicationBean1() {
     ...
     ArrayList<PowerDegationApproveModel> powerDelegationRequestModel= new ArrayList<PowerDegationApproveModel>();
    String requesterID;
  ...
  PowerDelegationApproveReportManager powerApproveManger=new PowerDelegationApproveReportManager();
    ...
    public ArrayList<PowerDegationApproveModel> getPowerDelegationRequestModel() {
    ...
    public void setPowerDelegationRequestModel(ArrayList<PowerDegationApproveModel> powerDelegationRequestModel) {
    ...
    public void setRequesterRank(String requesterRank) {
      ...
      private boolean populatePowerDelegationTableComponents() {
         int tableSize;
         ArrayList<HashMap> disciplineHashMap = powerApproveManger.getPowerDelegationApproveReportList();
        if (disciplineHashMap != null) {
         ...
         // disciplineReport.clear();
            for (HashMap hmDl : disciplineHashMap) {
                ...
                delegateFullName = hmDl.get("DelegatorFirst_Name").toString()+""+hmDl.get("Delegator_Middle_Name").toString()+""+hmDl.get("Delegator_last_Name").toString();
                delegateID = hmDl.get("DelegatorID").toString();
                delegatePosition = hmDl.get("Delegator_JobTitle").toString();
                delegateDepartment = hmDl.get("DELEGATOR_DEPARTMENT").toString();
                ...
                delegationApproveDay= hmDl.get("APPROVEDDATE").toString();
                approverID= hmDl.get("ApproverID").toString();
                approverFullName = hmDl.get("approverFirstName").toString()+" "+hmDl.get("approverMiddleName").toString()+" "+hmDl.get("approverLastName").toString();
                approverDepartment = hmDl.get("departmentName").toString();
                approverPosition=hmDl.get("approverJobPosition").toString();
                
                powerDelegationRequestModel.add(new PowerDegationApproveModel(requesterID,requesterFullName,requesterDepartment,requesterPosition,requesterRank,delegateFullName,delegateID,delegatePosition,delegateDepartment,delegatedSatrtDay,delegateEndDay,delegatedPosition,delegatedRank,delegationReasoun,delegationApproveDay,approverID,approverFullName,approverDepartment,approverPosition));
                tableSize++;
                    }
            return true;
        } else {
