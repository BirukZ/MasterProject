...
public class RequestEmployeeFromAnnualLeave extends AbstractPageBean {
   ...
   // private DepartmentManage departmentManage = new DepartmentManage();
    ...
    public void init() {
        if (callMeOnece) {
            lstEmployeeReturnFromAnualLeave.setRendered(false);
            userRole();
            callMeOnece = false;
...
//            }
    ...
    void employeeInformation(String leaveId) {
        checkReturnFromLeave = false;
        HashMap list = leaveManager.readEmployeeLeaveWithProfile(leaveId);
        if (list != null) {
            btnNew.setDisabled(false);
            ...
            lblJobPosition.setValue(list.get("JOB_NAME").toString());
    ...
    public void slbEmployee_processValueChange(ValueChangeEvent vce) {
        try {
            ...
            if (vce.getNewValue() != null) {
                ...
                employeeId=valueId[0];
                employeeInformation(valueId[1]);
...
//
    ...
    private void loadEmployeeInLeave() {
