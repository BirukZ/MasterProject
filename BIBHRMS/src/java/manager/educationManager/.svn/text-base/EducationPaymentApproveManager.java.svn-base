...
import entity.educationEntity.EducationPaymentApproveEntity;
 ...
 * @author mekete
 */
public class EducationPaymentApproveManager {
    ...
    EducationPaymentApproveEntity educationPaymentApproveEntity = new EducationPaymentApproveEntity();
    ...
    public static final String PROCESS_EDUCATION_PAYMENT = AuthorizationManager.PROCESS_EDUCATION_PAYMENT;//"5"
    public static final String FINAL_STATE_EDUCATION_PAYMENT = AuthorizationManager.readFinalState(PROCESS_EDUCATION_PAYMENT);
    public static final String INITIAL_STATE_EDUCATION_PAYMENT = AuthorizationManager.readInitialState(PROCESS_EDUCATION_PAYMENT);
...
//    }
    ...
    public boolean saveDecision(int requestId, String processedBy ,String decision,String currentState,String commnetGiven) {
        String nextState = (decision.contains("$$")) ? currentState : AuthorizationManager.readNextState(currentState, AuthorizationManager.PROCESS_EDUCATION_PAYMENT, decision);
        ...
        try {
            educationPaymentApproveEntity.insertEducationPaymentDecision(educationPaymentModel);
            return true;
        } catch (Exception ex) {
            ex.printStackTrace();
    ...
    public ArrayList<HashMap> getTableInfo(int reqestID,String date) {
        try {
            ...
            OracleCachedRowSet ocrs = educationPaymentApproveEntity.readAllTableInfo(reqestID,date);
            while (ocrs.next()) {
...
//                resultInfo.put("CGPA", ocrs.getString("CGPA"));
    ...
    public ArrayList<Option> getRequestsToBeProcessed() {
        ...
        try {
            java.sql.ResultSet _rs = educationPaymentApproveEntity.selectApprovableRequest(userName);
            while (_rs.next()) {
                pendingRequests.add(new Option(_rs.getString("EDUC_PAYMENT_REQUEST_ID") + "--" + _rs.getString("EDUCATION_REQUEST_ID") + "--" + _rs.getString("REQUESTER_ID") + "--" + _rs.getString("REQUEST_STATUS")+ "--" + _rs.getString("APPLIED_DATE"), _rs.getString("REQUESTER_ID") + " ON " + _rs.getString("APPLIED_DATE")));
            }
            _rs.close();
   ...
   public ArrayList<RequestHistoryModel> getRequestHistory(int requestId) {
        try {
         ...
         ResultSet _rs =  educationPaymentApproveEntity.selectProcessedOverTimeRequest(requestId);
    ...
    public ArrayList<Option> getMyDecisionsOnRequests() {
        ArrayList<Option> pendingRequests = new ArrayList<Option>();
        try {
            java.sql.ResultSet _rs = educationPaymentApproveEntity.selectMyDecisionsOnRequest(userName);
            while (_rs.next()) {
                pendingRequests.add(new Option(_rs.getString("EDUC_PAYMENT_REQUEST_ID") + "--" + _rs.getString("EDUCATION_REQUEST_ID") + "--" + _rs.getString("REQUESTER_ID") + "--" + _rs.getString("REQUEST_STATUS")+ "--" + _rs.getString("APPLIED_DATE"), _rs.getString("REQUESTER_ID") + " ON " + _rs.getString("APPLIED_DATE")));
            }
            _rs.close();
    ...
    public boolean UpadtechangeInstitutionRequest(int adminstratId, boolean selected) {
        ...
        try {
            educationPaymentApproveEntity.updateEducationRequest(educationPaymentModel);
        } catch (Exception ex) {
            ex.printStackTrace();
            ErrorLogWriter.writeError(ex);
            return false;
   ...
   public ArrayList<HashMap> getCourseInfo(int reqestID,String date) {
        try {
            ArrayList<HashMap> IntSuccLists = new ArrayList<HashMap>();
            OracleCachedRowSet ocrs = educationPaymentApproveEntity.readAllCourse(reqestID,date);
            while (ocrs.next()) {
                HashMap resultInfo = new HashMap();
//                resultInfo.put("AMINSTRATIVE_COST_TYPE", ocrs.getString("AMINSTRATIVE_COST_TYPE"));
                resultInfo.put("ID", ocrs.getString("ID"));
...
//            return null;
//        }
    public boolean updateReciptNumber(int educPaymentrequestId, String docRefNumber, String receiptNumber) {
        //String timeStamp = new Timestamp((new Date()).getTime()).toString();

//        educationPaymentModel = new EducationPaymentModel(adminstratId, docRefNumber);
        try {
            educationPaymentApproveEntity.updateReceiptNumber(educPaymentrequestId,  docRefNumber, receiptNumber);
        } catch (Exception ex) {
            ex.printStackTrace();
            ErrorLogWriter.writeError(ex);
            return false;
