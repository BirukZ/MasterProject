...
import manager.disciplineManager.DisciplineRequestReportManager;
 ...
 */
public class DisciplineRequestReport extends AbstractPageBean {
     ...
     */
    public DisciplineRequestReport() {
    }
    ...
    public void prerender() {
      //populateCourseTableComponents();
       ...
       populateDisiciplineTableComponentsByMonth();
  ...
  // populateDisiciplineTableComponentsByMonth(month);
   ...
   DisciplineRequestReportManager requestReportManager=new DisciplineRequestReportManager();
   ArrayList<DisciplineReportModel> disciplineReport= new ArrayList<DisciplineReportModel>();
    ...
    public ArrayList<DisciplineReportModel> getDisciplineReport() {
        return disciplineReport;
    }


    public void setDisciplineReport(ArrayList<DisciplineReportModel> disciplineReport) {
    ...
    // populateDisiciplineTableComponentsByMonth(month)
      ...
      private boolean populateCourseTableComponents() {
         int tableSize;
         ArrayList<HashMap> disciplineHashMap = requestReportManager.getDisciplineInfo();
        if (disciplineHashMap != null) {
            tableSize = 0;
         // disciplineReport.clear();
            for (HashMap hmDl : disciplineHashMap) {
                //requesterId = educatRequestId;
                requesterID = hmDl.get("emp_id").toString();
                requesterFullName = hmDl.get("first_name").toString()+""+hmDl.get("middle_name").toString()+""+hmDl.get("last_name").toString();
                requesterPosition = hmDl.get("job_description").toString();
                requestrDepartment = hmDl.get("department_name").toString();
                ...
                disciplineHappenDay = hmDl.get("offence_date").toString();
                disciplineType = hmDl.get("offence_name").toString();
                disciplineReportDay = hmDl.get("reported_date").toString();
                disciplineDescripition = hmDl.get("description_of_offence").toString();
                requesterInterset=hmDl.get("requested_measure_to_be_taken").toString();
                requesterEyewitnessInside = hmDl.get("witness_type").toString();
                requesterEyewitnessOutSide = hmDl.get("witness_type_other").toString();
                problemSeenBy = hmDl.get("forwarded_to_committee").toString();
                commiteeCommentes = hmDl.get("CommentesGiven").toString();
                lastApprovedPenality = hmDl.get("approved_penality").toString();
                approvedDeduction=hmDl.get("approved_deduct").toString();
                approvedDuration=hmDl.get("approved_duration").toString();
                 approvedDate = hmDl.get("approvedDate").toString();
                disciplineReport.add(new DisciplineReportModel(requesterID, requesterFullName, requesterPosition , requestrDepartment, offenderID, offenderFullName, offenderPosition,offenderDepartment,disciplineHappenDay,disciplineType,disciplineReportDay,disciplineDescripition, requesterInterset, requesterEyewitnessInside,requesterEyewitnessOutSide,problemSeenBy, commiteeCommentes,lastApprovedPenality,approvedDeduction,approvedDuration,approvedDate));
                tableSize++;
                    }
            return true;
        } else {
       ...
       private boolean populateDisiciplineTableComponentsByMonth() {
           
         int tableSize;

         ArrayList<HashMap> disciplineHashMap = requestReportManager.getDisciplineInfoByMonth();
        if (disciplineHashMap != null) {
            tableSize = 0;
