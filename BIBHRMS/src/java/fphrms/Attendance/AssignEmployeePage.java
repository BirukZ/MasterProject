...
import manager.attendanceManager.AssignEmployeeManager;
 ...
 */
public class AssignEmployeePage extends AbstractPageBean {
    ...
    private AssignEmployeeManager assignEmployeeManager = null;
    private ArrayList<SelectItem> listOfBoss = new ArrayList<SelectItem>();
    private EmployeeManage employeeManage = null;
    ...
    public void setListOfBoss(ArrayList<SelectItem> listOfBoss) {
        this.listOfBoss = listOfBoss;
    }
...
//                            hm.get("EMP_STATUS").toString()));
    ...
    public String btnSaveGroup_action() {
        if (txtEmployeeId.getValue() != null) {
            if (assignEmployeeManager.saveEmployeeInUnderBoss(txtEmployeeId.getValue().toString(),
                    employeeGroupForInsert())) {
                showSuccessOrFailureMessage(true, "Saved  successfully Done.");
            } else {
        ...
        for (int i = 0; i < getAvailableEmployeeInDataModel().size(); i++) {
            if (getAvailableEmployeeInDataModel().get(i).isSelected()) {
                getAvilableEmployeeInGroup().add(new AvilableEmployeeInGroup(
                        ...
                        txtEmployeeId.getValue().toString(), null, "01"));
    ...
    public String btnAddAll_action() {
        for (int i = 0; i < getAvailableEmployeeInDataModel().size(); i++) {
            getAvilableEmployeeInGroup().add(new AvilableEmployeeInGroup(
                    getAvailableEmployeeInDataModel().get(i).getEmployeeID(),
                    getAvailableEmployeeInDataModel().get(i).getEmployeeName(),
                    null, txtEmployeeId.getValue().toString(), "01"));
            getAvailableEmployeeInDataModel().remove(i);
            i--;
            rowCounterForAssignedUser++;
        }
        Collections.sort(getAvilableEmployeeInGroup(), new Comparator<AvilableEmployeeInGroup>() {

            public int compare(AvilableEmployeeInGroup s1, AvilableEmployeeInGroup s2) {
...
//return null means stay on the same page
    ...
    public String btnAddEmployee_action() {
        try {
            if (DEPARRTMENT_ID != 0) {
                createObject();
                ArrayList<HashMap> employeeList = assignEmployeeManager.listOFEmployeeInDepartment(Integer.toString(DEPARRTMENT_ID));
    ...
    private void createObject() {
        if (assignEmployeeManager == null) {
            assignEmployeeManager = new AssignEmployeeManager();
        }
    }
    ...
    public String btnWithOutBoss_action() {
        createObject();
        ArrayList<HashMap> employeeList = assignEmployeeManager.listOFEmployeeNotAssign();
        for (HashMap hm : employeeList) {
            getAvailableEmployeeInDataModel().add(new AvilableEmployeeDataModel(
                    hm.get("EMPLOYEEID").toString(),
                    hm.get("FULL_NAME").toString(),
    ...
    public String btnSearch_action() {
        try {
            if (HRValidation.validateRequired(txtEmployeeId.getValue())) {
                isnull();
                displayListOfEmployeeInThis(txtEmployeeId.getValue().toString());
            } else {
                showSuccessOrFailureMessage(false, "Employee  assigned Into Location  successfully Done.");
            }
        } catch (Exception e) {
        }
        //return null means stay on the same page
    ...
    private void displayListOfEmployeeInThis(String bossId) {
        try {
            if (HRValidation.validateRequired(bossId)) {
                isnull();
                lblEmployeeFullName.setValue(employeeManage.employeeFullName(bossId));
                getAvilableEmployeeInGroup().clear();
                getAvailableEmployeeInDataModel().clear();
                createObject();
                ArrayList<HashMap> employeeLists = assignEmployeeManager.listOFEmployeeByImmediateBoss(bossId);
                for (HashMap hm : employeeLists) {
                    getAvilableEmployeeInGroup().add(new AvilableEmployeeInGroup(
                            hm.get("EMPLOYEEID").toString(),
                            hm.get("FULL_NAME").toString(),
                            hm.get("ID").toString(),
                            hm.get("BOSSID").toString(),
                            hm.get("EMP_STATUS").toString()));
                }
            } else {
                showSuccessOrFailureMessage(false, "Can't be Employee Id is empty");
            }
        } catch (Exception e) {
        }
    }

    public String txtEmployeeName_action() {
        selectedId = null;
        selectedId = ApplicationBean1.getSelectedEmployeeId();
        if (selectedId != null) {
            txtEmployeeId.resetValue();

            loadInformation(ApplicationBean1.getSelectedEmployeeId());
        }
        //return null means stay on the same page
        return null;
    }

    void loadInformation(String immedateBossId) {
        isnull();
        lblEmployeeFullName.setValue(employeeManage.employeeFullName(immedateBossId));
        getAvilableEmployeeInGroup().clear();
        getAvailableEmployeeInDataModel().clear();
        createObject();
        ArrayList<HashMap> employeeLists = assignEmployeeManager.listOFEmployeeByImmediateBoss(immedateBossId);
        for (HashMap hm : employeeLists) {
            getAvilableEmployeeInGroup().add(new AvilableEmployeeInGroup(
                    hm.get("EMPLOYEEID").toString(),
                    hm.get("FULL_NAME").toString(),
        ...
        txtEmployeeId.setValue(immedateBossId);
    }

    private void listOfBossReLOader() {
        getListOfBoss().clear();
        setListOfBoss(assignEmployeeManager.getListOfBoss());
    }

    public void selectOneListbox1_processValueChange(ValueChangeEvent vce) {
        try {
            if (vce.getNewValue() != null) {
                displayListOfEmployeeInThis(vce.getNewValue().toString());
            }
        } catch (Exception e) {
        }
