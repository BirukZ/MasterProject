...
public class EducationPostPaymentRequestManager {
    ...
    public boolean saveCourseTaking(ArrayList<EducationPaymentModel> courseTakingList, String appliedDate) {
        ArrayList<EducationPaymentModel> Insereted = new ArrayList<EducationPaymentModel>();
        ...
        try {
            for (int counter = 0; counter < courseTakingList.size(); counter++) {
                ...
                if (courseTakingList.get(counter).getDatabaseStatus().equalsIgnoreCase("New")) {
                   Insereted.add(courseTakingList.get(counter));
                } else if (courseTakingList.get(counter).getDatabaseStatus().equalsIgnoreCase("Edited")) {
                    toBeUpdated.add(courseTakingList.get(counter));
                } else if (courseTakingList.get(counter).getDatabaseStatus().equalsIgnoreCase("Deleted")) {
                    toBeDeleted.add(courseTakingList.get(counter));
                }
            }
            EducationPostPaymentRequestEntity.insertCourseTakingList(Insereted,appliedDate);
...
public boolean deleteBasicLevelRequest(int educationRequestId) {
        try {
            EducationPostPaymentRequestEntity.deleteCourseBasicLevel(educationRequestId);
            return true;
        } catch (Exception ex) {
            ex.printStackTrace();
            ErrorLogWriter.writeError(ex);
...
//                committeeList.add(currentCommittee);
    ...
    public ArrayList<SelectItem> getResultType() {
        ...
        try {
            committeeTypes.add(new SelectItem(null, "--Select Your Result--"));
